...
namespace intprog2.Auth
{
...

    {
        ...
        public override async Task GrantResourceOwnerCredentials(OAuthGrantResourceOwnerCredentialsContext context)
        {
            ...
            var ogrservis = new ogrservis();
            var ogr = ogrservis.ÖğrenciOturumAc(context.UserName, context.Password);
            List<string> ogrYetkileri = new List<string>();
            ...
            if ( ogr != null)
            {
               ...
               ogrYetkileri.Add(yetki);

                var identity = new ClaimsIdentity(context.Options.AuthenticationType);

                identity.AddClaim(new Claim(ClaimTypes.Name, context.UserName));
                identity.AddClaim(new Claim(ClaimTypes.Role, yetki));
                identity.AddClaim(new Claim(ClaimTypes.PrimarySid, ogr.ogrId.ToString()));

                AuthenticationProperties propert = new AuthenticationProperties(new Dictionary<string, string>
                {
                    { "ogrId",ogr.ogrId.ToString() },
                    { "ogrKadi", ogr.ogrAdsoyad },
                    { "ogrYetkileri",Newtonsoft.Json.JsonConvert.SerializeObject(ogrYetkileri) }
